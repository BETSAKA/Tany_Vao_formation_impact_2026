...
### Exploration des Données geoBoundaries

Pour illustrer l'accès aux données spatiales administratives, nous allons utiliser les données geoBoundaries. Ces données sont disponibles à différents niveaux administratifs (niveaux 0, 1, 2, etc.). Voici ce que nous allons faire :

1. Téléchargement manuel : Rendez-vous sur le site https://www.geoboundaries.org/ et explorez les différentes options de téléchargement des données administratives. Cela vous permettra de vous familiariser avec les types de niveaux disponibles.

2. Téléchargement via R : Utilisons la fonction gb_get_adm() du package geobounds pour obtenir directement les données dans R.

```{r}
# Télécharger les données geoBoundaries pour Madagascar au niveau administratif 0
gb_mada0 <- geobounds::gb_get_adm0(country = "Madagascar")
```

Exercices : 

- Visualisez les données `gb_mada0` téléchargées avec tmap. De quel niveau administratif s'agit-il ? 
...
# Calculer l'intersection entre la bbox du raster et gb_mada0
intersection <- st_intersection(raster_bbox, gb_mada0)


# Réduire l'étendue du raster avec crop() en utilisant l'emprise de Madagascar
gfc_raster_crop <- crop(gfc_raster, intersection)
plot(gfc_raster_crop)
...
1. **Exploration Manuelle** : Rendez-vous sur le site [Global Forest Change 2023](https://storage.googleapis.com/earthenginepartners-hansen/GFC-2023-v1.11/download.html), et lisez la documentation pour comprendre la structure des fichiers. Téléchargez une tuile de votre choix (utilisez `download.file()` si vous travaillez en ligne. 
...
Maintenant, nous allons appliquer une variante sous forme de tests logique aux communes (niveau 4 dans geoBoundaries) pour déterminer quelles communes contiennent des aires protégées.
...
communes <- geobounds::gb_get_adm4(country = "Madagascar") %>%
  st_make_valid()
...
```{r}
# Différence entre la frontière nationale (geoBoundaries) et les aires protégées
non_protected <- st_difference(gb_mada0, union_result)
